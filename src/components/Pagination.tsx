            ...
            <div className="max-w-[50vw] overflow-clip">
                <div className="flex items-center gap-3 w-max">
                    {Array.from({length: pageSize}, (_, item) => 
                        <div onClick={() => setPage(item + 1)} key={item} className={`${(item + 1) == meta?.current_page? "bg-blue-600 text-white" : ""} h-[40px] w-[40px] flex items-center justify-center rounded border`}>
                            {item + 1}
                        </div>
                    )}
                </div>
